...
import org.indritbreti.restprocessordemo.exceptions.api.httpCore.HttpRequestMethodNotSupportedException;
import org.indritbreti.restprocessordemo.exceptions.to_delete.ErrorResponse__;
    ...
    public ResponseEntity<Object> handleConstraintViolation(TransactionSystemException ex, WebRequest request) {
        ...
        if (ex.getCause() instanceof RollbackException) {
            ...
            if (rollbackException.getCause() instanceof ConstraintViolationException) {
                return new org.indritbreti.restprocessordemo.exceptions.to_refactor.ConstraintViolationException(HttpStatus.BAD_REQUEST, ex.getMessage()).toResponseEntity();
            }
        }
...
//        return new CustomTypeMismatchException();
        if (ex instanceof MethodArgumentTypeMismatchException ex_) {
            return new org.indritbreti.restprocessordemo.exceptions.api.customExceptionHandlerBaseExceptions.MethodArgumentTypeMismatchException(ex_).toResponseEntity();
        }
        // TODO possible check for ConversionNotSupportedException

        return new org.indritbreti.restprocessordemo.exceptions.api.customExceptionHandlerBaseExceptions.TypeMismatchException(ex).toResponseEntity();
    }
